...
- Los archivos se sirven **solo** por la API con permiso de Financiero (`GET /api/financials/records/:id/documents/:documentId/file`, `?download=1` para descargar), con `Content-Type` detectado al subir, `nosniff` y sin caché. Nunca por URL pública del bucket. El frontend los pide con el token y abre un blob local.
