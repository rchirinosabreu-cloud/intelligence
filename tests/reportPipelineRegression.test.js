...
test('report pipeline regressions', async (t) => {
  ...
  await t.test('fallback narratives name the client and vary interpretation by chart type', async () => {
    ...
    assert.doesNotMatch(result.sections.map(section => section.narrativeComment).join(' '), /este dato permite identificar/i);
    assert.match(result.sections[0].narrativeComment, /Reels.*20.*Fotos.*5/s);
    assert.match(result.sections[1].narrativeComment, /300.*100/s);
