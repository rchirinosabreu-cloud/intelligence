...
test('pipeline status is publicly reachable before API authentication', () => {
  const indexSource = readFileSync(new URL('../src/routes/index.js', import.meta.url), 'utf8');
  const publicStatus = indexSource.indexOf("router.get('/reports/pipeline-status'");
  const authGate = indexSource.indexOf('router.use(authenticateToken)');
  assert.ok(publicStatus >= 0, 'public pipeline status route must be registered');
  assert.ok(publicStatus < authGate, 'pipeline status must be registered before authentication');
  assert.doesNotMatch(indexSource, /import reportsRouter,\s*\{[^}]+\}\s*from '.\/api\/reports\.js'/, 'startup must not depend on named exports from reports router');
});
