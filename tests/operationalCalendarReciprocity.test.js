...
test('production startup restores calendar invitation schema idempotently', async () => {
  ...
  assert.match(packageJson, /ensure-calendar-invitations-schema\.js/);
  assert.match(bootstrap, /ADD COLUMN IF NOT EXISTS "captureWithFireflies"/);
  assert.match(bootstrap, /ADD COLUMN IF NOT EXISTS "attendeeEmails"/);
  assert.match(bootstrap, /ADD COLUMN IF NOT EXISTS "googleConnectionId"/);
  assert.match(bootstrap, /CREATE TABLE IF NOT EXISTS "GoogleCalendarEventLink"/);
});
