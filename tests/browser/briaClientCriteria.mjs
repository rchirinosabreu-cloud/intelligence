...
try {
  ...
  for (const [name, width, height, dark] of [['desktop', 1366, 1000, false], ['mobile', 390, 844, false], ['tablet', 768, 1024, false], ['mobile-dark', 390, 844, true], ['small-mobile', 320, 568, false], ['landscape', 844, 390, false]]) {
    ...
    await page.route('**/api/**', async route => {
      ...
      assert.equal(body.reason, body.text === 'Usar español de Colombia.' ? 'Guía editorial vigente.' : undefined);
      const created = { ...body, id: `new-${result.criteria.length}`, version: 1, status: 'PROPOSED', history: [{ action: 'PROPOSE', actorName: 'Responsable de ejemplo', at: new Date().toISOString(), version: 1, reason: body.reason || '' }] };
    ...
    await dialog.getByRole('textbox', { name: 'Criterio', exact: true }).fill('Evitar promesas de resultados garantizados.');
    assert.equal(await dialog.getByLabel('Por qué debe recordarlo Bria').count(), 0);
    assert.equal(await dialog.getByLabel('Contexto o fuente (opcional)', { exact: true }).isVisible(), false);
    assert.equal(await save.isEnabled(), true);
    await page.evaluate(() => { document.documentElement.style.fontSize = '200%'; });
    assert.ok(await dialog.evaluate(element => element.scrollWidth <= element.clientWidth + 1), `${name}: proposal reflows at 200%`);
    await page.evaluate(() => { document.documentElement.style.fontSize = ''; });
    await page.screenshot({ path: `output/bria-criteria-proposal-${name}.png`, fullPage: true });
    fail = true;
    await save.click();
    await dialog.getByRole('alert').waitFor();
    assert.equal(await dialog.getByRole('textbox', { name: 'Criterio', exact: true }).inputValue(), 'Evitar promesas de resultados garantizados.');
    fail = false; hold = {};
    await save.click();
    await page.waitForFunction(() => document.querySelector('button[disabled][data-saving="true"]'));
    assert.equal(result.criteria.length, 1, 'a click alone must not create the proposal');
    hold.resolve(); hold = null;
    await dialog.getByRole('heading', { name: 'Lo que Bria debe recordar', exact: true }).waitFor();
    await dialog.getByText('Evitar promesas de resultados garantizados.').waitFor();
    const newArticle = dialog.locator('article').filter({ hasText: 'Evitar promesas de resultados garantizados.' });
    await newArticle.getByRole('button', { name: 'Ver detalle', exact: true }).click();
    await newArticle.locator('summary').filter({ hasText: 'Historial' }).click();
    await newArticle.getByText('Sin contexto añadido.', { exact: true }).waitFor();
    await dialog.getByRole('button', { name: 'Proponer criterio', exact: true }).click();
    await dialog.getByRole('textbox', { name: 'Criterio', exact: true }).fill('Usar español de Colombia.');
    const contextToggle = dialog.locator('summary').filter({ hasText: 'Añadir contexto' });
    await contextToggle.focus(); await page.keyboard.press('Enter');
    const contextField = dialog.getByRole('textbox', { name: 'Contexto o fuente (opcional)', exact: true });
    await contextField.fill('  Guía editorial vigente.  ');
    assert.equal(await contextField.getAttribute('required'), null);
    const contextBounds = await contextField.boundingBox(), toggleBounds = await contextToggle.boundingBox();
    assert.ok(contextBounds.y >= toggleBounds.y + toggleBounds.height + 12, `${name}: context has breathing room`);
    await page.screenshot({ path: `output/bria-criteria-context-${name}.png`, fullPage: true });
    await contextToggle.click();
    assert.equal(await contextField.isVisible(), false);
    await save.click();
    await dialog.getByRole('heading', { name: 'Lo que Bria debe recordar', exact: true }).waitFor();
    const withContext = dialog.locator('article').filter({ hasText: 'Usar español de Colombia.' });
    await withContext.getByRole('button', { name: 'Ver detalle', exact: true }).click();
    await withContext.locator('summary').filter({ hasText: 'Historial' }).click();
    await withContext.getByText('Guía editorial vigente.', { exact: true }).waitFor();
