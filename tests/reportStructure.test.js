...
test('organic summary contains only the four approved headline metrics', () => {
  const scoped = buildScopedReportData([
  ...
  assert.deepEqual(Object.keys(scoped.organicSummary), ['follows', 'views', 'interactions', 'reachOrganic']);
  assert.equal(scoped.organicSummary.views.value, 27064);
  assert.equal(scoped.organicSummary.interactions.value, 844);
  assert.equal(scoped.organicSummary.viewers, undefined);
  assert.equal(scoped.organicSummary.profileVisits, undefined);
  assert.equal(scoped.organicSummary.linkClicks, undefined);
