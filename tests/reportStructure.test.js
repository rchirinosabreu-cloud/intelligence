...
test('organic summary is one non-zero group with organic semantics only', () => {
  const scoped = buildScopedReportData([
    ...
    { sourceId: 'instagram', sectionCategory: 'ORGANIC', platform: 'INSTAGRAM', screenType: 'CONTENT_SUMMARY', metrics: { viewsOrganic: metric(27064, 'Visualizaciones orgánicas'), interactions: metric(844, 'Interacciones') } },
    { sourceId: 'cross', sectionCategory: 'ORGANIC', platform: 'CROSS_PLATFORM', screenType: 'METRIC_TRENDS', metrics: { linkClicks: metric(64, 'Clics'), profileVisits: metric(825, 'Visitas'), ctr: metric(0, 'CTR') } },
  ]);
  assert.deepEqual(Object.keys(scoped.organicSummary), ['viewsOrganic', 'videoViews', 'interactions', 'linkClicks', 'profileVisits']);
  assert.equal(scoped.organicSummary.viewsOrganic.value, 27064);
  assert.equal(scoped.organicSummary.views, undefined);
  assert.equal(scoped.organicSummary.spend, undefined);
  assert.equal(scoped.organicSummary.reachPaid, undefined);
  assert.equal(scoped.organicSummary.follows, undefined);
});
