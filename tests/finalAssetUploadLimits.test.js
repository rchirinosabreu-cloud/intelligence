...
import {
  FINAL_ASSET_MAX_BYTES, FINAL_ASSET_MAX_FILES, FINAL_ASSET_MAX_TOTAL_BYTES,
  checkFinalAssetSelection, fileTooLargeMessage, formatFileSize, tooManyFilesMessage, tooMuchAtOnceMessage
...
test('a normal video fits, and what does not fit is explained in words', () => {
  ...
  assert.equal(
    checkFinalAssetSelection([{ name: 'campaña.mov', size: 140 * MB }]),
  ...
  assert.equal(checkFinalAssetSelection(heavy), tooMuchAtOnceMessage(270 * MB), 'el envío completo también tiene tope');
...
test('the limit lives in one place: the route, the storage and the screen read the same number', async () => {
  ...
  assert.match(screen, /const problem = checkFinalAssetSelection\(files\);/, 'la pantalla avisa antes de gastar la subida');
