...
export const DIAGNOSTIC_QUERIES = {
  ...
  calendar_meetings_by_type_and_organizer_last_12: `SELECT type, COALESCE("organizerEmail", '(sin organizador)') AS organizer, COUNT(*)::int AS events, COUNT(*) FILTER (WHERE "meetingLink" IS NOT NULL)::int AS with_meeting_link FROM "OperationalEvent" WHERE "googleCancelled" = false AND "isAllDay" = false AND "startAt" >= now() - interval '12 weeks' AND "startAt" <= now() GROUP BY 1, 2 ORDER BY 3 DESC LIMIT 15`,
  calendar_meetings_with_link_last_10_days: `SELECT type, COUNT(*)::int AS events_with_link, MIN("startAt") AS first, MAX("startAt") AS last FROM "OperationalEvent" WHERE "meetingLink" IS NOT NULL AND "googleCancelled" = false AND "isAllDay" = false AND "startAt" >= now() - interval '10 days' AND "startAt" <= now() GROUP BY 1 ORDER BY 2 DESC`,
  ...
  // What actually matters: how long after a meeting ENDS its minute is ready.
  // `meetingAt` is the start, so the duration has to be added before comparing.
  lag_after_meeting_ends_since_live: `SELECT COUNT(*)::int AS n, ROUND((percentile_cont(0.5) WITHIN GROUP (ORDER BY EXTRACT(EPOCH FROM ("processedAt" - ("meetingAt" + make_interval(secs => COALESCE("durationSeconds", 0)))))/60))::numeric, 1) AS p50_min, ROUND((MAX(EXTRACT(EPOCH FROM ("processedAt" - ("meetingAt" + make_interval(secs => COALESCE("durationSeconds", 0)))))/60))::numeric, 1) AS max_min FROM "MeetingMinute" WHERE status = 'READY' AND "processedAt" IS NOT NULL AND "deletedAt" IS NULL AND "durationSeconds" IS NOT NULL AND "meetingAt" >= '${MINUTES_AUTOMATION_LIVE_SINCE}'`,
  ...
  observer_by_grounding: `SELECT COALESCE(grounding, '(anterior al cambio)') AS grounding, COUNT(*)::int AS n, COUNT(*) FILTER (WHERE status IN ('OPEN','REVIEWED','SNOOZED'))::int AS activas FROM "BriaObserverSignal" GROUP BY 1 ORDER BY 2 DESC`,
  ...
  plans_failed_detail: `SELECT id, "briaReviewAttempts", LEFT("briaReviewError", 120) AS error, "briaReviewDiagnostics" -> -1 ->> 'code' AS last_cause, "briaReviewDiagnostics" -> -1 ->> 'status' AS last_status, "briaReviewStartedAt" FROM "ContentPlan" WHERE "briaReviewState" = 'FAILED' ORDER BY "briaReviewStartedAt" DESC NULLS LAST LIMIT 10`,
  reviews_by_trigger: `SELECT trigger, status, COUNT(*)::int AS n FROM "ContentPlanReview" GROUP BY 1, 2 ORDER BY 1, 2`,
  reviews_with_usage: `SELECT COUNT(*) FILTER (WHERE usage IS NOT NULL)::int AS with_usage, COUNT(*)::int AS reviews, MIN("completedAt") FILTER (WHERE usage IS NOT NULL) AS first_with_usage, COALESCE(SUM((usage -> 'totals' ->> 'inputTokens')::bigint), 0)::bigint AS input_tokens, COALESCE(SUM((usage -> 'totals' ->> 'outputTokens')::bigint), 0)::bigint AS output_tokens, COALESCE(SUM((usage -> 'totals' ->> 'calls')::int), 0)::int AS calls FROM "ContentPlanReview" WHERE "completedAt" >= now() - interval '7 days'`,
  ...
  findings_open_by_age: `SELECT CASE WHEN "firstDetectedAt" >= now() - interval '7 days' THEN '1: 0-7 dias' WHEN "firstDetectedAt" >= now() - interval '30 days' THEN '2: 7-30 dias' ELSE '3: mas de 30 dias' END AS age, COUNT(*)::int AS open_findings FROM "ContentPlanReviewFinding" WHERE status = 'OPEN' GROUP BY 1 ORDER BY 1`,
