...
- **Consejos y recordatorios personalizados:** `lib/dashboardTips.js` elige uno por persona y día (Bogotá): primero los ligados a su situación real (vencidas, devueltas, reunión de hoy, seguimientos de CRM vencidos, muchas para hoy, buen ritmo), y en días tranquilos un consejo de plataforma dos de cada tres días. Se pinta con `dashboard/DashboardTip.jsx` arriba del bloque de recordatorios; «Ocultar por hoy» se guarda en `localStorage` del dispositivo. Los consejos solo hablan de módulos que la persona puede abrir y nunca inventan cifras: leen el payload del dashboard. Nuevos consejos van al catálogo `PLATFORM_TIPS`, cortos y sin nombrar proveedores de IA.
