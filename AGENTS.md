...
- El dashboard saluda por nombre y muestra el resumen de tareas en cinco tarjetas (Activas y Devueltas con degradado, Para hoy, Vencidas y Logros hoy en color sólido; nada de amarillo en los indicadores). Distribución: columna principal con Anuncios a todo el ancho, debajo los recordatorios personales («Tu CRM pide atención», Mis clientes) a la izquierda y Próximos pendientes agrupados por día a la derecha (a todo el ancho si no hay recordatorios), y Asignar cliente según rol; columna derecha con Logros recientes al lado de Anuncios (misma altura) y Reuniones debajo. **Radar de Foco y Reto de la semana se retiraron** del dashboard por decisión de Rodny; `focusCards` y `weeklyHabit` siguen en el payload para otros consumidores, no volver a pintarlos sin aprobación.
