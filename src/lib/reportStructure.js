...
const ORGANIC_SUMMARY_KEYS = [
  'viewsOrganic', 'views', 'videoViews', 'viewers', 'reach', 'reachOrganic',
  'interactions', 'linkClicks', 'profileVisits', 'follows',
];
...
const buildOrganicSummary = (sources) => {
  ...
  for (const source of sources) {
    for (const [key, metric] of Object.entries(cleanMetrics(source.metrics))) {
      if (!ORGANIC_SUMMARY_KEYS.includes(key) || Number(metric.value) <= 0) continue;
      ...
      if (!current || sourcePriority(source) > current._priority) {
        candidates[key] = { ...metric, _priority: sourcePriority(source), sourceId: source.sourceId };
      }
    }
  }
