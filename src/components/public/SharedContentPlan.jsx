...
const FinalAssetPreview = ({ assets = [] }) => {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="overflow-hidden rounded-[2rem] border border-zinc-100 bg-zinc-50 dark:border-white/10 dark:bg-white/5">
      ...
      {assets.length > 1 && (
        <div className="flex max-w-full gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" aria-label="Navegación del carrusel">
          {assets.map((entry, index) => (
            <button key={entry.id || entry.url || index} type="button" onClick={() => setActiveIndex(index)} aria-label={`Ver lámina ${index + 1}`} aria-current={index === activeIndex ? 'true' : undefined} className={`h-2.5 rounded-full transition-all ${index === activeIndex ? 'w-7 bg-violet-600' : 'w-2.5 bg-zinc-300 dark:bg-zinc-700'}`} />
          ))}
        </div>
      )}
    </div>
