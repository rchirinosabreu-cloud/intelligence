...
import { buildMetricReportCreateData } from '../../services/reportPersistence.js';
...
router.post('/extract-metrics', upload.any(), async (req, res) => {
    try {
        ...
        let report;
        try {
            const data = buildMetricReportCreateData({
                clientId,
                periodKind,
            ...
            console.error('[Reports API] Prisma insertion failed', {
                name: dbError?.name,
                code: dbError?.code,
                meta: dbError?.meta,
                message: String(dbError?.message || dbError).split('\n').filter(Boolean).at(-1)?.slice(0, 500)
            });
            return res.status(500).json({
                error: 'Database validation or insertion error',
                code: dbError?.code || 'REPORT_PERSISTENCE_FAILED'
            });
        }
